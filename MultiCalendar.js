...
class MultiCalendar {
    constructor() {
        ...
        this.jal_week_days = ["شنبه", "یکشنبه", "دوشنبه", "سه شنبه", "چهار شنبه", "پنج شنبه", "جمعه"];
        this.jal_week_days_abbr = ["ش", "ی", "د", "س", "چ", "پ", "ج"];
        this.jal_months = ["فروردین", "اردیبهشت", "خرداد", "تیر", "مرداد", "شهریور", "مهر", "آبان", "آذر", "دی", "بهمن", "اسفند"];

        this.greg_week_days = ["Saturday", "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday"];
        this.greg_week_days_abbr = ["Sat", "Sun", "Mon", "Tue", "Wed", "Thr", "Fri"];
        this.greg_months = ["January", "February", "March", "April", "May", "June", "July", "August", "Septamber", "October", "November", "December"];
        this.greg_months_abbr = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
        this.greg_months_fa = ["زانويه", "فوريه", "مارچ", "آوريل", "مي", "ژون", "جولاي", "آگوست", "سپتامبر", "اکتبر", "نوامبر", "دسامبر"];
    }
    ...
    getJalDaysInMonth(gdp) {
        ...
        let year = this.getJalYearFrom(gdp);
        let month = this.getJalMonthFrom(gdp);
        let is_leap = this.IsJalLeapYear(jal_year);
        let M = [];
    ...
    getGdp(Year, Month, DayOfMonth) {
        ...
        if (this.IsGregLeapYear(Year)) M = [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
    ...
    jalDateToGdp(Year, Month, DayOfMonth) {
        ...
        if (this.IsJalLeapYear(y)) M = [31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, 30];
        else M = [31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, 29];
    ...
    jalDateStrToGdp(dateStr) {
        ...
        if (jal_date == null) {
            ...
            let gdp = this.jalDateToGdp(jal_date.year, jal_date.month, jal_date.day);
            return gdp;
        }
    }
    ...
    getGregDateFromGdp(gdp) {
        ...
        res = this.getGregMonth(this.IsGregLeapYear(Y), Rem);
        let M = res.GregMonth;
